...
客户端日志管理器，专为浏览器环境设计，使用 console API
输出日志，提供多级别日志、彩色输出、调试模式控制等功能。
...
- **消息长度**：可通过 `maxMessageLength` 限制单条消息长度（默认
  32KB），超出部分截断并加省略标记。
- **输出异常隔离**：控制台输出包在 try/catch
  中，若控制台抛错（如展开循环引用对象）不会导致应用中断。
...
- **生产环境建议**：在生产环境中，建议设置 `debug: false`
  完全禁用日志输出，避免性能影响
...
客户端日志器经过全面测试，包含 21 个浏览器环境测试用例，全部通过。测试使用
@dreamer/test 浏览器集成在真实浏览器环境中验证所有功能。
