...
| 项目             | 值                                     |
| ---------------- | -------------------------------------- |
| **Logger 版本**  | @dreamer/logger@1.1.0                  |
| **服务容器版本** | @dreamer/service@^1.1.0                |
| **测试框架**     | @dreamer/test@^1.2.3                   |
| **测试日期**     | 2026-07-22                             |
| **测试环境**     | Deno 2.9+、Bun 1.3+、Node.js 22+       |
| **测试文件数**   | 2                                      |
| **总用例数**     | 89（Deno）/ 86（Bun）/ 67（Node）      |
| **通过率**       | 100% ✅                                |
| **执行时间**     | 约 23s（Deno）/ 23s（Bun）/ 1s（Node） |

### 三端测试摘要

三端全部通过。Deno 与 Bun 运行完整套件（含 Playwright 浏览器测试）；Node 仅运行
`mod.test.ts`（浏览器测试需 Chromium + esbuild，按项目规范在 Node CI 中排除）。

- **Deno（89 条）**：68 条服务端（`mod.test.ts`）+ 21 条浏览器
  （`client.test.ts`），通过 @dreamer/test 无头浏览器集成执行。
- **Bun（86 条）**：65 条服务端 + 21 条浏览器。
- **Node.js 22（67 条）**：仅 `mod.test.ts`，通过 `tsx --test` 执行。

## 测试结果

### 总体统计

- **测试总数**：89（Deno）/ 86（Bun）/ 67（Node）
- **通过**：89 / 86 / 67 ✅
- **失败**：0
- **通过率**：100% ✅
- **执行时间**：约 23s（Deno）/ 23s（Bun）/ 1s（Node）
...
| `mod.test.ts`    | 68     | ✅ 全部通过 | 服务端单元测试 + LoggerManager + ServiceContainer 集成测试 |
...
- ✅ 默认显示时间戳、支持禁用/启用、**color: "auto"**（按 TTY 与 format
  自动检测）、**output.console: "auto"**（接受配置；无 TTY 时写文件需 close
  防泄漏）、text/color 下时间戳控制、JSON 格式行为
...
4. ✅ **时间戳与级别**：showTime、showLevel 等配置有专门用例；**color:
   "auto"**、**output.console: "auto"** 有专门用例（已去掉 output.auto）
...
@dreamer/logger 共 89 个测试用例，全部通过，通过率 100%。

- **浏览器测试 (client.test.ts)**：21 ✅
- **服务端测试 (mod.test.ts)**：68 ✅

**配置**：当前版本已去掉 `output.auto`，统一使用 `output.console: "auto"`
表示自动输出目标；`color` 与 `output.console` 均支持 `"auto"`
值。详见上文「配置变更说明」。
