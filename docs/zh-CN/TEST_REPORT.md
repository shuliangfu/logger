...
| **总用例数**     | 89                              |
| **通过率**       | 100% ✅                         |
| **执行时间**     | 约 4 秒（Deno，`deno test -A`） |

### 配置变更说明（与本次测试一致）

- **已去掉** `output.auto` 参数，自动模式改为使用
  **`output.console: "auto"`**（前台打控制台、后台写文件）。
- **`color`** 支持 **`"auto"`**：未设或为 `"auto"` 时按 TTY 与 format
  自动检测（前台有颜色，后台无颜色）。
- **`output.console`** 支持 **`"auto"`**：与上述自动输出目标行为一致。
