...
First stable release. Logging library compatible with Deno and Bun. Pure TypeScript, no external dependencies.

#### Server-side Logger

- **Log levels**: debug, info, warn, error, fatal with dynamic level control
- **Formats**: text (human-readable), JSON (structured), color (console only, env-aware)
...
- **Log rotation**: By size, by time, or size-time combined, with optional compression
...
- **Performance monitoring**: `startPerformance`, `endPerformance`, `performance` decorator
...
- `redirectConsoleToLogger()`: Forward console.log/info/warn/error/debug to logger
...
- **ServiceContainer**: Integration with `@dreamer/service` for dependency injection
- **Methods**: `getLogger`, `createLogger`, `hasLogger`, `removeLogger`, `getLoggerNames`, `setLevel`, `close`
